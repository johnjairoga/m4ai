...
- **Modelo → à vista:** um único `\n` — **sem** linha em branco entre a linha do modelo e `Apenas R$...`.
- **À vista → parcelado:** `\n\n` — **uma** linha em branco entre o valor à vista e `Ou Nx... no cartão`.
- **Entre blocos de orçamento diferentes** no mesmo elemento (ex.: vários modelos na mesma faixa de preço): `\n\n` entre o fim de um bloco completo e o início do próximo.
...
**⚠️ PROIBIDO:**
- ❌ Linha em branco entre modelo e `Apenas R$...` no mesmo bloco (ex.: `\n\n` entre modelo e à vista) — poluição visual.
- ❌ Linha em branco entre `Apenas R$...` e `Ou Nx...` no mesmo bloco — deve haver **exatamente uma** linha em branco (`\n\n`) entre à vista e cartão.
